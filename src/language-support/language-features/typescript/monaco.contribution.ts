...
let typescriptSetup = false;
let javascriptSetup = false;

function setupTypeScriptMode() {
  if (!typescriptSetup) {
...
function setupJavaScriptMode() {
  if (!javascriptSetup) {
    javascriptSetup = true;
    return getMode().then((mode) => mode.setupJavaScript(javascriptDefaults));
  }
}

languages.onLanguage('typescript', setupTypeScriptMode);
languages.onLanguage('typescriptreact', setupTypeScriptMode);

languages.onLanguage('javascript', setupJavaScriptMode);
languages.onLanguage('javascriptreact', setupJavaScriptMode);
