...
export function setupTypeScript(defaults: LanguageServiceDefaults): void {
  typeScriptWorker = setupMode(defaults, ['typescript', 'typescriptreact']);
}

export function setupJavaScript(defaults: LanguageServiceDefaults): void {
  javaScriptWorker = setupMode(defaults, ['javascript', 'javascriptreact']);
}
...
function setupMode(
  ...
  const client = new WorkerManager(modeId[0], defaults);
  ...
  function registerProviders(): void {
    ...
    if (modeConfiguration.diagnostics) {
      providers.push(
        new languageFeatures.DiagnosticsAdapter(libFiles, defaults, modeId[0], worker)
      );
    }
  }
