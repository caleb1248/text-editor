...
for (const extension of extensions) {
  ...
  if (contributes.languages) {
    for (const language of contributes.languages) {
      if (language.configuration && !assetMap.has(language.configuration)) {
        const asset = `new URL(${JSON.stringify(language.configuration)}, import.meta.url).href`;
        assetMap.set(language.configuration, asset);
      }
    }
  }

  for (const key in contributes) {
    if (!['grammars', 'languages'].includes(key)) {
      contributes[key] = undefined;
    }
  }
...
for(const language of contributes.languages || []) {
  if(typeof language.configuration==='string'&&assetMap.has(language.configuration)){
    language.configuration=assetMap.get(language.configuration);
  }
}
assetMap.clear();
...
`;
  ...
  for (const [assetPath] of assetMap) {
    ...
    if (assetPath.endsWith('.json')) {
      // Strip comments and minify JSON files
      const content = await readFile(srcPath, 'utf-8').then((text) =>
...
let entryPointContent = `// Auto-generated by scripts/compile-languages.ts

// Basic languages

${languageDirs.map((dir) => `import './basic-languages-out/${dir}/manifest.js';`).join('\n')}
